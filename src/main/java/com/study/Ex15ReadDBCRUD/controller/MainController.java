...
import com.study.Ex15ReadDBCRUD.entity.MemberRepository;
...
public class MainController {
    ...
    public String joinAction(@ModelAttribute MemberJoinDto dto){
        ...
        return "<script>alert('회원가입 성공');location.href='/admin';</script>";
    }
    //viewDTO?id=1
    ...
    public String deleteMember(@RequestParam int id){
        ...
        if(!optional.isPresent()){
            ...
            return "<script>alert('회원정보조회 실패'); hisory.back();</script>";
        }
        ...
        try{
            ...
            e.printStackTrace();
            return "<script>alert('회원정보삭제 실패'); hisory.back();</script>";
        }
